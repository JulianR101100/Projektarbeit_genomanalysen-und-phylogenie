...
# 1. Weitere Datein sollten geordnet in verschiedenen Ordnern liegen (-> database_blast_BLOSUM90, logs_BLOSUM90)
mkdir -p database_blast_BLOSUM90 logs_BLOSUM90
...
-out database_blast_BLOSUM90/database \
> logs_BLOSUM90/makeblastdb.log 2>&1
...
-db database_blast_BLOSUM90/database \
-outfmt "7 delim=	 sacc length pident gaps score bitscore evalue" \
-matrix BLOSUM90 \
> query_seq_blast_BLOSUM90.blast

echo "BLAST search mit BlOSUM90 allen möglichen Zielsequenzen durchgeführt und Ergebnisse in 'query_seq_blast_BLOSUM90.blast' gespeichert."
# head -n 100 query_seq_blast_BLOSUM90.blast